...
def dashboard():
    ...
    applicants = supabase.table('applicants').select('id, full_name, student_id, email, status, applied_at').execute()
    ...
    for row in rows:
        try:
            d = row['applied_at'][:10]
            activity_map.setdefault(d, {'date': d, 'newApplications': 0})
            activity_map[d]['newApplications'] += 1
        except Exception:
            pass
    recent_activity = sorted(activity_map.values(), key=lambda x: x['date'])[-30:]

    # Recent applicants (last 5, any status)
    recent_applicants = sorted(rows, key=lambda x: x.get('applied_at', ''), reverse=True)[:5]

    # Students: reports and evaluations status
    today = datetime.now(timezone.utc).date()
    students_data = supabase.table('students').select(
        'id, work_terms(id, report_deadline, status, term_reports(id), job_assignments(id, evaluations(id), exception_flags(id)))'
    ).execute().data or []
    ...
    for s in students_data:
        for wt in (s.get('work_terms') or []):
            deadline = wt.get('report_deadline')
            reports = wt.get('term_reports') or []
            jobs = wt.get('job_assignments') or []
            has_report = len(reports) > 0
            has_eval = any(len(j.get('evaluations') or []) > 0 for j in jobs)

            if has_report:
                reports_submitted += 1
            elif deadline and str(today) > deadline:
                missing_reports += 1
            ...
            for j in jobs:
                if j.get('exception_flags'):
                    flagged_students.add(s['id'])

    enrolled = supabase.table('students').select('id', count='exact').execute().count or 0
    ...
    return jsonify({
        ...
        'missingEvaluations': missing_evals,
        'evaluationsSubmitted': evals_submitted,
        'flaggedStudents': len(flagged_students),
    })
