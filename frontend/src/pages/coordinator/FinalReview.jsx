...
export default function FinalReview() {
  ...
  async function confirmAction() {
    ...
    try {
      ...
      setToast({ type: 'success', msg: confirm.status === 'finally_accepted' ? `${confirm.name} has been finally accepted and added to the roster.` : `${confirm.name} has been finally rejected.` })
      setConfirm(null)
      load()
    } catch (err) {
      ...
      {confirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div role="dialog" aria-modal="true" className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-2">Confirm Action</h2>
            ...
            <p className="text-gray-600 text-sm mb-5">
              ...
              {confirm.status === 'finally_accepted'
                ? ' They will be added to the official roster.'
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 text-white py-2 rounded-lg font-medium text-sm disabled:opacity-50 ${confirm.status === 'finally_accepted' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-500 hover:bg-red-600'}`}
              >
                {actionLoading ? 'Processing…' : 'Confirm'}
              </button>
              <button
                onClick={() => setConfirm(null)}
                className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg font-medium text-sm hover:bg-gray-200"
              >
                Cancel
              </button>
            </div>
